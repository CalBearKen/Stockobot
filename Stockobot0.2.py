...
"""Stockobot v0.2, doesn't do much yet
...
def initial_setup():
    ...
    '''Design a way for the function to check whether or not this robot has been initialized before, try using a config text'''
    #initialized = open('init.txt', 'r')
...
def step_sell(symbol, increment, quantity): #increment: dollar price increase per order ||, expiration=5
    price0 = robin.stocks.get_latest_price(symbol)
    price0 = float(price0[0])
    #today = date.today()
    #expirationDate = today + timedelta(days=expiration)
    #expirationDate = today.strftime('%Y-%m-%d')
    for order in range(1, quantity+1):
        #strike = price0 + increment*(order-.5)
        price =  price0 + order*increment
        #receipt = (strike, price, expirationDate, strike)
        data = robin.orders.order_sell_limit(symbol, 1, limitPrice=price, timeInForce='gfd')
        print(data)
